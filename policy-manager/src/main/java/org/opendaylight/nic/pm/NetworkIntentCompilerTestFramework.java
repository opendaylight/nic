...
/**
 * A test framework which listens for policy changes by becoming a
 * {@link PolicyListener} and feeding those policy changes to its own
 * {@link PolicyFramework} and {@link PolicyService}. This test framework
 * creates and populates its own {@link EndpointService} and
 * {@link DeviceService} with hosts (99.1.2.1-10) and a single device/switch.
 * <P>
 * In the future, these services should be populated via listening to the
 * MD-SAL. Specifically, L2switch's host discovery would populate
 * {@link EndpointService} and OpenFlow-plugin would populate
 * {@link DeviceService}.
 *
 * @author Shaun Wackerly
 */
public class NetworkIntentCompilerTestFramework implements PolicyListener,
        AutoCloseable {

    private static final Logger log = LoggerFactory
            .getLogger(NetworkIntentCompilerTestFramework.class);
    ...
    private final PolicyFrameworkImpl framework; // FIXME: Split into
                                                 // PolicyService,
                                                 // PolicyFramework
    ...
    public void notifyChange(Set<Policy> created, Set<Policy> updated,
            Set<Policy> removed) {
        log.info("Received " + created.size() + " created, " + updated.size()
                + " updated, " + removed.size() + " removed");
        ...
        for (Policy p : updated) {
            // FIXME: We need to retrieve the old version of the policy before
            // the update
        ...
        log.info("Starting compile with " + framework.getPolicies().size()
                + " policies.");
    ...
    void recompile() {
        ...
        List<CompiledPolicy> cops = compiler.compile(framework.getPolicies(),
                es.getAll(), framework.getActions(), framework.getTermTypes(),
                ds.getAll());
        ...
        log.info("Finished policy compilation, generated " + flows.size()
                + " flows");
        if (flows.size() < 100) {
            ...
            for (Term t : e.getTerm()) {
                ...
                if (t.getRange() != null) {
                    for (Range r : t.getRange()) {
                        intervals.add(IntervalImpl.getInstance(r.getStart()
                                .getValue(), r.getEnd().getValue()));
                    }
                }
     ...
     *            endpoint IP
     * @param port
     *            endpoint port (on dummyDevice)
     * @param vlan
     *            endpoint vlan (on dummyDevice)
     * @return endpoint
     */
    private static IpEndpoint makeEndpoint(String ip, Integer port, Integer vlan) {
        ...
        try {
            ep1 = new IpEndpoint(InetAddress.getByName(ip), new PortImpl(port),
                    new SegmentId(vlan), dummyDevice);
