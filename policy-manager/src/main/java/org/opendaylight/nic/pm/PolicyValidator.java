...
/**
 * Validates policies that have been added to the CONFIGURATION tree and once
 * validated moves them to the OPERATIONAL tree.
 *
...
public class PolicyValidator implements AutoCloseable, DataChangeListener {
    ...
    private static final Logger log = LoggerFactory
            .getLogger(PolicyValidator.class);
     ...
     * which has the given ID. Note that the presence of an identifier does not
     * necessarily indicate the presence of the stored policy. An instance
     ...
     */
    private InstanceIdentifier<Policy> getInstanceId(DomainId dId, AppId aId,
            PolicyId pId) {
    ...
    private Policy readPolicy(DomainId dId, AppId aId, PolicyId pId) {
        ...
        // Get the path which uniquely identifies (via a reference) the new
        // policy we'll add
        ...
        final CheckedFuture<Optional<Policy>, ReadFailedException> readFuture = readTrans
     ...
     *            policy to write
     */
    private void writePolicy(DomainId dId, AppId aId, PolicyId pId,
            Policy policy) {
        log.debug("WRITE: dom={} app={} pol={} policy={}", dId, aId, pId,
                policy);
        if (policy == null) {
        ...
        // Get the path which uniquely identifies (via a reference) the policy
        // we'll write
        final InstanceIdentifier<Policy> path = getInstanceId(dId, aId, pId);
        ...
        for (Map.Entry<InstanceIdentifier<?>, DataObject> entry : change
            ...
            for (Application app : dom.getApplication()) {
                for (AppPolicy pol : app.getAppPolicy()) {
                    writePolicy(dom.getDomainId(), app.getAppId(),
                            pol.getPolicyId(), pol.getPolicy());
                }
            }
        }
