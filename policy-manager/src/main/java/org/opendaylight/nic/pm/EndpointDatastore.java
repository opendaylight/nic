...
/**
 * A temporary framework for storing endpoints and their attributes.
 * Before Lithium release, this should be replaced by a proper
 * implementation of {@link EndpointService}.
 *
 * @author Shaun Wackerly
 */
public class EndpointDatastore implements EndpointService, AutoCloseable, DataChangeListener {

    private static final Logger log = LoggerFactory.getLogger(EndpointDatastore.class);
     ...
     *
     * @param as application service
     * @param dataBroker data broker
     * @param dev device to which all endpoints will be assigned
     */
    public EndpointDatastore(ApplicationService as,
                             DataBroker dataBroker,
                             Device dev,
                             NetworkIntentCompilerTestFramework nicFramework) {
        delegate = new EndpointServiceImpl(as);
        dummyDevice = dev;
        this.nicFramework = nicFramework;

        InstanceIdentifier<Endpoint> path = InstanceIdentifier.builder(Endpoints.class).child(Endpoint.class).toInstance();
     ...
     */
    private class MyEndpointId implements org.opendaylight.nic.intent.EndpointId {
     ...
     */
    private class MyEndpoint extends org.opendaylight.nic.intent.impl.IpEndpoint {
