...
@Command(name = "map", scope = "intent", description = "List current state of the mapping service."
...
public class IntentAddMappingShellCommand extends OsgiCommandSupport {
    ...
    @Option(name = "-l", aliases = {
            "--list" }, description = "List Mapping Service.\n-l / --list <regular expression> [ENTER]", required = true, multiValued = false)
    String list = "";

    @Option(name = "-p", aliases = {
            "--print" }, description = "Print Mapping Service.\n-p / --print [ENTER]", required = false, multiValued = false)
    boolean print = false;
    ...
    private void printContents(StringBuilder builder, String key) {
        builder.append(key).append(" = {\n").append(service.stringRepresentation(key)).append("}\n");
    }
}
