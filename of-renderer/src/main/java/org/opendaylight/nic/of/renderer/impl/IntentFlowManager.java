...
public class IntentFlowManager extends AbstractFlowManager {
    ...
    public void setSubjectsMapping(
    ...
    // endPointGroups list received from Intent operation is a list of mac addresses
    // For each mac address as key access the mapping information from mapping-service
    ...
    private void createMplsMatch(
            List<String> endPointGroups, Map<String, Map<String, String>> subjectsMapping,
            MatchBuilder matchBuilder) {

        for (String macAddress : endPointGroups) {
            if (subjectsMapping.containsKey(macAddress)) {
                Long mplsLabel = new Long(subjectsMapping.get(macAddress).get(MPLS_LABEL_KEY));
