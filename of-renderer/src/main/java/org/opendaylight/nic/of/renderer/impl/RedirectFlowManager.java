...
public class RedirectFlowManager extends AbstractFlowManager implements PacketProcessingListener, AutoCloseable {
    ...
    /**
     * Redirect flow entry
     * @param redirectNodeCache :redirectNodeCache contains redirect node details
     */
    public void redirectFlowEntry(Map<String, RedirectNodeData> redirectNodeCache) {
        LOG.trace("Redirect flow NodeId's {}", redirectNodeCache.values());
        Collection<RedirectNodeData> redirectNodeDataList =  redirectNodeCache.values();
        for (RedirectNodeData nodeData : redirectNodeDataList) {
            if (!nodeData.toString().contains("null")) {
    ...
    private void generateRedirectFlows(List<String> endPointGroups, String sourceNodeConnectorId,
        ...
        if (shortestPath != null && !shortestPath.isEmpty()) {
            for (Link link: shortestPath) {
                ...
                String linkPrevTargetTp = link.getDestination().getDestTp().getValue();
                if (shortestPath.size() == 1) {
                    pushRedirectFlow(endPointGroups, linkSourceNodeId, sourceNodeConnectorId,  linkSourceTp, flowAction);
                    pushRedirectFlow(endPointGroups, linkTargetNodeId, linkTargetTp, targetNodeConnectorId, flowAction);
                      ...
                      if (sourceNodeId.getValue().equals(linkSourceNodeId.getValue())) {
                          ...
                          pushRedirectFlow(endPointGroups, linkTargetNodeId, linkPrevTargetTp, targetNodeConnectorId, flowAction);
                      } else {
    ...
    private void pushRedirectFlow(List<String> endPointGroups, NodeId nodeId,String inPort,
        ...
        LOG.info("pushRedirectFlow on Node: {}", nodeId.getValue());
    ...
    private void addMacNodeToCache(String macAddress, String nodeId) {
        ...
        for (RedirectNodeData redirectNodeData : redirectNodeDataList) {
            ...
            if (!redirectNodeData.toString().contains("null")) {
                redirectFlowEntry(redirectNodeCache);
            }
       }
    }
