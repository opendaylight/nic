...
public class NicProvider implements NicConsoleProvider {
    ...
    public String compile(int flag) {
        ...
        CompilerGraph compiler = new CompilerGraphImpl(mappingSvc);
        ...
        for (Intent intent : intents) {
            // create a collection of source nodes
            Nodes sourceContainer = (Nodes) intent.getSubjects().get(0).getSubject();
            // create a collection of destination nodes
            Nodes destinationContainer = (Nodes) intent.getSubjects().get(1).getSubject();
            // create a collection of action nodes
            // TODO the getAction() only gets the action fields
            Action actionContainer =
                    (Action) intent.getActions().get(0).getAction();
            String sourceSubject = sourceContainer.getName();
            String destinationSubject = destinationContainer.getName();
            Set<Nodes> sources;
            try {
                sources = translateSubject(compiler, sourceSubject);
            } catch (UnknownHostException e) {
        ...
        try {
            // TODO Remove Directed Graph JUNG dependency
            compiledPolicies = (Collection<InputGraph>) compiler.compile(policies);
        ...
        // As a start return the inputGraphs
        return stringBuilder.toString();
    }
    ...
    private String formatPolicies(Collection<InputGraph> policies, int flag) {
        ...
        for (InputGraph policy : policies) {
            stringBuilder.append(policies.toString());
            stringBuilder.append('\n');
        }
        return stringBuilder.toString();
    }
