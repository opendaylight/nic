...
public class GBPTenantPolicyCreator {
    ...
    private TenantBuilder getTenant() {
        ...
        forwardContextBuilder.setL3Context(ImmutableList.copyOf(l3Contexts)).setL2BridgeDomain(ImmutableList.copyOf(l2BridgeDomains))
        .setL2FloodDomain(ImmutableList.copyOf(l2FloodDomains)).setSubnet(ImmutableList.of(
        ...
        policyBuilder.setContract(ImmutableList.of(this.getDefaultContract()));
        policyBuilder.setSubjectFeatureInstances(new SubjectFeatureInstancesBuilder()
    ...
    private Boolean getTenantEndpointAttributes(String subjectId, Boolean provider) {
        ...
        for (Endpoint endpoint : matchingEndpoints) {
            ...
            for (L3Address address : endpoint.getL3Address()) {
                ...
                } else if (!GBPRendererHelper.contains(consumerL3Addresses, address)) {
    ...
    private List<Endpoint> readEPNodes(String subjectId) {
        ...
        if (node != null) {
            for (Endpoint endpoint : node.getEndpoint()) {
                if (endpoint.getEndpointGroup().getValue().equalsIgnoreCase(subjectId)) {
                    matchingGroup.add(endpoint);
                }
            }
        } else {
