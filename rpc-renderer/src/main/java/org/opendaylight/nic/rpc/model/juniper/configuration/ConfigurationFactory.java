...
public class ConfigurationFactory {
    ...
    public synchronized Map<DeviceDetails, String> generateCompiledCommitByDevice() {
    ...
    public synchronized Map<DeviceDetails, String> generateCommitByDevice() {
        final Map<DeviceDetails, String> commitByDevice = Maps.newConcurrentMap();
        configurationsByIp.keySet().forEach(ip -> {
            final Configuration configuration = configurationsByIp.get(ip).iterator().next();
            final String commit = configuration.generateCommitmentConfig(configuration.generateRPCStructure());
            commitByDevice.put(
                    deviceDetailsByIp.get(ip),
                    commit);
        });
    ...
    public synchronized void orderSingleConfigurationByDevice(final Configuration configuration,
                                                              final DeviceDetails deviceDetails,
                                                              final Boolean isADeleteSchema) {
        this.isADeleteSchema = isADeleteSchema;
        Preconditions.checkNotNull(configuration);
        Preconditions.checkNotNull(deviceDetails);
        deviceDetailsByIp.put(deviceDetails.getHttpIp(), deviceDetails);
        configurationsByIp.put(deviceDetails.getHttpIp(), Sets.newHashSet(configuration));
    }
