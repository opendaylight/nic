...
public class NicProviderTest {
    ...
    public void setup() throws Exception {
        ...
        when(mockDataBroker.newWriteOnlyTransaction()).thenReturn(
                mockWriteTransaction);
        when(mockDataBroker.newReadOnlyTransaction()).thenReturn(
                mockReadOnlyTransaction);
    ...
    public void testInit() {
        ...
        when(FrameworkUtil.getBundle(nicProvider.getClass())).thenReturn(
                mockBundle);
        when(mockBundle.getBundleContext()).thenReturn(mockBundleContext);
        when(
                mockBundleContext.registerService(NicConsoleProvider.class,
                        nicProvider, null)).thenReturn(
                mockNicConsoleRegistration);
        /**
         * Here verifying init() should initialize operational and default
         * config data in MD-SAL data store.
         */
        nicProvider.init();
        assertEquals(mockNicConsoleRegistration,
                nicProvider.nicConsoleRegistration);
        ...
        verify(mockBundleContext, times(1)).registerService(
                NicConsoleProvider.class, nicProvider, null);
    }
    ...
    public void testInitIntentsOperational() {
        ...
        verify(mockListenableFuture, times(1)).addListener(isA(Runnable.class),
                isA(Executor.class));
    }
    ...
    public void testListIntents() throws Exception {
        ...
        when(mockIntents.getIntent()).thenReturn(
                mockListOfIntentsForConfiguration);
        ...
        when(mockIntentsTwo.getIntent()).thenReturn(
                mockListOfIntentsForOperational);
        when(mockOptionalTwo.get()).thenReturn(mockIntentsTwo);
        when(mockCheckedFutureTwo.checkedGet()).thenReturn(mockOptionalTwo);
        when(
                mockReadOnlyTransaction.read(LogicalDatastoreType.OPERATIONAL,
                        NicProvider.INTENTS_IID)).thenReturn(
                mockCheckedFutureTwo);
    ...
    public void testGetIntent() throws Exception {
        Intent actualIntent;
        Uuid uuid = Uuid
                .getDefaultInstance("b9a13232-525e-4d8c-be21-cd65e3436037");
        /**
         * Here creates required mock objects and defines mocking functionality
         * for mock objects.
         */
        ...
        when(mockOptional.get()).thenReturn(mockIntentForConfiguration, null,
                mockIntentForOperational);
        when(mockCheckedFuture.checkedGet()).thenReturn(mockOptional);
        when(
                mockReadOnlyTransaction.read(
                        eq(LogicalDatastoreType.CONFIGURATION),
                        isA(InstanceIdentifier.class))).thenReturn(
                mockCheckedFuture);
        when(
                mockReadOnlyTransaction.read(
                        eq(LogicalDatastoreType.OPERATIONAL),
    ...
    public void testCompile() throws Exception {
        ...
        when(
                mockIntentCompiler.createPolicy(isA(Set.class), isA(Set.class),
                        isA(Set.class))).thenReturn(mockPolicy);
        PowerMockito.stub(
                PowerMockito.method(IntentCompilerFactory.class,
        ...
        when(mockSubjects.getSubject()).thenReturn(mockEndPointGroupForSource,
                mockEndPointGroupForDestination);
        when(mockIntent.getSubjects()).thenReturn(listOfSubjects);
        when(mockActions.getAction()).thenReturn(mockAllow, mockBlock,
                mockAction, mockBlock);
        ...
        when(mockEndPointGroupTwo.getName())
                .thenReturn("Mock EndPointGroupTwo");
        when(mockEndPointGroupForSource.getEndPointGroup()).thenReturn(
                mockEndPointGroup);
        when(mockEndPointGroupForDestination.getEndPointGroup()).thenReturn(
                mockEndPointGroupTwo);
         ...
         */
        expectedResult = "[ERROR] Invalid action: "
                + mockAction.getClass().getName();
        actualResult = nicProvider.compile();
        assertEquals(expectedResult, actualResult);
        /**
        ...
        when(mockIntentCompilerException.getMessage()).thenReturn(
                "mockito msg..");
        when(mockIntentCompilerException.getRelatedPolicies()).thenReturn(
                listOfPolicy);
        doThrow(mockIntentCompilerException).when(mockIntentCompiler).compile(
        ...
        when(mockSubjects.getSubject()).thenReturn(mockEndPointGroupForSource,
                mockEndPointGroupForDestination);
        when(mockIntentCompiler.parseEndpointGroup("Mock EndPointGroupTwo"))
        ...
        when(mockSubjects.getSubject()).thenReturn(mockEndPointGroupForSource,
                mockEndPointGroupForDestination);
        when(mockIntentCompiler.parseEndpointGroup(eq("Mock EndPointGroupOne")))
                .thenThrow(new UnknownHostException(""));
    ...
    public void testFormatPolicies() throws Exception {
        ...
        actualResult = Whitebox.invokeMethod(nicProvider, "formatPolicies",
                listOfPolicy);
        assertEquals(expectedResult, actualResult);
    }
}
