...
public class NicProvider implements NicConsoleProvider {
    ...
    public String compile() {
        ...
        if (endpointManager.getNodeMap() == null) {
        }

        if (endpointManager.getNodeMap().size() == 0) {
        }

        // print out nodes
        for (Node key : endpointManager.getNodeMap().keySet()) {

            System.out.println(key.toString() + " HOSTS"
                    + endpointManager.getNodeMap().get(key).toString());
