...
public class NicProvider implements NicConsoleProvider {
    ...
    public String compile(boolean showGraph) {
        ...
        try {
            PolicyGenerator generator = new PolicyGenerator(compiledPolicies, new EndpointManager().getEndpointMap());
            pushCompiledPoliciesToDevice(generator.generate(), null);
        }
        catch(Exception e){
    ...
    private  InstructionBuilder createAllowInstruction(InstructionBuilder ib) {
    ...
    private  InstructionsBuilder createBlockInstruction(InstructionsBuilder ib) {

        // Wrap our Apply Action in an Instruction
        ib.setInstruction(null);
    ...
    private void pushCompiledPoliciesToDevice(Collection<org.opendaylight.nic.impl.Policy> policies, NodeId nodeId){
        for(org.opendaylight.nic.impl.Policy policy : policies){

            InstructionsBuilder instructionBuilder = new InstructionsBuilder();
            MatchBuilder matchBuilder = new MatchBuilder();

            // Create generic Flow object
            String flowIdStr = "Flow_" + UUID.randomUUID().toString();
            FlowBuilder flowBuilder = new FlowBuilder();
            flowBuilder.setId(new FlowId(flowIdStr));
            FlowKey key = new FlowKey(new FlowId(flowIdStr));
            flowBuilder.setBarrier(true);
            flowBuilder.setTableId((short)100);
            ...
            if(policy.action().equals(ACTION_ALLOW)) {
                flowBuilder.setInstructions(createBlockInstruction(instructionBuilder).build());
            }
            else {
                flowBuilder.setInstructions(createBlockInstruction(instructionBuilder).build());
            }

            //flowBuilder.setMatch(createSrcL3IPv4Match(MatchBuilder).build(), new Ipv4Prefix());
