...
/**
 * Generates native protocol formats, based upon compiled policies. The registered
 * set of translators will be used to translate compiled policies into
 * a native protocol format.
 *
 * @param N native protocol output type
 * @param A native type required for translating actions
 * @param B native builder required for translating terms
 * @author Shaun Wackerly
 */
public interface CodeGenerator<N,A,B> {

    /**
     * Registers the given action translator to be used when translating
     * actions to OpenFlow as a native protocol.
     *
     * @param trans action translator
     */
     ...
     * actions to OpenFlow as a native protocol.
     *
     * @param trans translator to register
     */
    void register(TermTranslator<B> trans);

    /**
     * Generates a set of native protocol objects which will implement
     * the behavior described by the policies on the given device.
     *
     * @param policies the policies to apply
     * @param dev the device to apply the policies against
     * @return the set of natively-typed objects
     */
    Set<N> generate(List<CompiledPolicy> policies, DeviceId dev);

    /**
     * Checks whether the given {@link CompiledPolicy} has been applied to
     * the given device.
     *
