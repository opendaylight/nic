    ...
    public boolean isMember(Endpoint endpoint) {
        ...
        if (!result.type().isAttribute()) {
            throw new IllegalStateException(
                    "cannot evaluate, wrong tokey type: " + result.type());
        }
        ...
        while (!q.isEmpty()) {
            ...
            if (tt.isAttribute()) {
                ...
                if (tt == TokenType.AND || tt == TokenType.OR) {
                    ...
                    if ((!left.type().isAttribute())
                            || (!right.type().isAttribute())) {
                        throw new IllegalStateException(
                                "bad token type in stack");
                    }
                    String s = String.format(" ( %s %s %s ) ", left.value(),
                            tt.toString(), right.value());
                    ...
                    String s = String.format(" ( %s %s ) ", tt.toString(),
                            n.value());
