...
public class PolicyImpl implements Policy {
    ...
    public static PolicyImpl createPolicyImpl(
            org.opendaylight.nic.intent.Policy policy,
            Map<TermLabel, TermType> termTypes) {
        return new PolicyImpl(policy.name(), policy.application(),
                policy.src(), policy.dst(), createClassifierImpl(
                        policy.classifier(), termTypes),
                policy.actionLabelToAuxDataMap(), policy.isExclusive());
    }

    private PolicyImpl(String policyName, Application application,
            EndpointGroup src, EndpointGroup dst, ClassifierImpl classifier,
            Map<ActionLabel, AuxiliaryData> map, boolean isExclusive) {
    ...
    private static ClassifierImpl createClassifierImpl(
            Classifier classifierRequest, Map<TermLabel, TermType> termTypes) {
        ...
        for (Expression er : classifierRequest.getExpressions()) {
            ...
            for (Term tr : er.getTerms()) {
                ...
                for (Interval i : tr.getIntervals()) {
                    IntervalImpl interval = IntervalImpl.getInstance(i.start(),
                            i.end());
                ...
                if (tt == null) {
                    throw new IllegalArgumentException(
                            "Couldn't find type for label: " + tr.typeLabel()
                                    + ", registered types are: "
                                    + termTypes.entrySet());
                }
                TermImpl term = TermImpl.getInstance(
                        termTypes.get(tr.typeLabel()), intervalCollection);
                termCollection.add(term);
            }
            ExpressionImpl expression = ExpressionImpl
                    .getInstance(termCollection);
            expressionCollection.add(expression);
        }

        ClassifierImpl classifier = ClassifierImpl
                .getInstance(expressionCollection);
        return classifier;
    }
