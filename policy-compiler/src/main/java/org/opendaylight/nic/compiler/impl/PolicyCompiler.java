...
/**
 * A policy compiler which compiles a set of {@link Policy}
 * objects into {@link CompiledPolicy} objects.
 *
...
public class PolicyCompiler {
	 ...
	 * using the given endpoints, actions, terms, and devices to aid the
	 * compilation process.
	 *
	 * @param policyRequests given set of policies
	 * @param endpoints given set of endpoints
	 * @param actions given set of actions
	 * @param terms given set of terms
	 * @param devices given set of devices
	...
	public List<CompiledPolicy> compile(Set<Policy> policyRequests,
		...
		for (Policy pr: policyRequests) {
			CompilerNode n = CompilerNode.createNode(pr, endpoints, actions, terms);
			nodeList.add(n);
		}

		//TODO for policies using service groups, check service group availability
		//     if not available, modify actions to use failure action

		//detect and resolve conflicts, creating orthogonal nodes in the process
		...
		for (CompilerNode cn : nodeList) {
            ...
            if ((cn.srcMembers().size()==1) && cn.srcMembers().equals(cn.dstMembers())) {
                continue;
            }
	        cops.add(cn);
		}
	...
	private void detectAndResolve(List<CompilerNode> list, Map<EndpointId, Endpoint>endpointMap) {
