...
class PrefixParser {
    ...
    public static Queue<Token> parse(List<Token> tokens) {
        ...
        for (Token token : tokens) {
            ...
            case RIGHTPAREN:
                if (!handleRightParentheses(queue, operatorStack, token)) {
                    throw new IllegalArgumentException(
                            "parse failure on right parenthesis");
                }
            ...
            default:
                throw new IllegalArgumentException(
                        "parse failure on illegal token");
            }
        }
        ...
        while ((topOfStack = operatorStack.peekLast()) != null) {
            if (!topOfStack.type().isOperator()) {
                throw new IllegalArgumentException(
                        "parse failure, expected operator, token: "
                                + topOfStack);
            }
    ...
    /**
     * Handles parsing of operands which are attributes by adding them to th
     * prefix output queue.
     * 
     * @param queue
     *            Prefix output queue
    ...
    /**
     * Handles parsing of operators (and, or, not), which is the incoming
     * {@link Token}
     * <p>
     * Moves operands, which are {@link Token}, from the stack to the queue
     * while either of these conditions are met:
     * <ul>
     * <li>incoming token is left associative and its priority is less than or
     * equal to the top of stack token priority</li>
     * <li>incoming token is right associative and its priority is less than the
     * top of stack token priority</li>
     * </ul>
     * <p>
     * After the operands are popped, the incoming operand {@link Token} is
     ...
     *            Token to handle
     */
    private static void handleOperators(Queue<Token> queue,
            LinkedList<Token> stack, Token token) {

        Token topOfStack;
        while (( // stack not empty
        (topOfStack = stack.peekLast()) != null)
                && // this check must be first to ensure non-null topOfStack
                (((token.type().associativity() == Associativity.LEFT) && (token
                        .type().priority() <= topOfStack.type().priority())) || ((token
                        .type().associativity() == Associativity.RIGHT) && (token
                        .type().priority() < topOfStack.type().priority())))) { // pop
                                                                                // top
                                                                                // of
                                                                                // stack
                                                                                // onto
    ...
    /**
     * Handles parsing of left parentheses by simply adding it to the operand
     * stack.
     * 
     * @param stack
     *            Operand stack
     ...
     *            Token to handle
     */
    private static void handleLeftParentheses(Queue<Token> queue,
            LinkedList<Token> stack, Token token) {
        stack.add(token);
    }

    /**
     * Handles parsing of right parentheses {@link Token} Moves operands from
     * the stack to the queue until a left parentheses is found.
     * 
     * @param queue
     *            Prefix output queue
     * @param stack
     ...
     * @return
     */
    private static boolean handleRightParentheses(Queue<Token> queue,
            LinkedList<Token> stack, Token token) {
        Token topOfStack;
        while (( // stack not empty
                (topOfStack = stack.peekLast()) != null)
                && // this check must be first to ensure non-null topOfStack
                ((!topOfStack.type().equals(TokenType.LEFTPAREN)))) { // pop top
                                                                      // of
                                                                      // stack
                                                                      // onto
                                                                      // queue
