...
public class TokenParser {
	...
	/**
	 * Creates a compiled regular expression {@link Pattern} out of {@link TokenType}
	 */
	private TokenParser() {
    ...
    private List<Token> tokenParse(String attributeExpression) {
    	...
    	String expression = attributeExpression.replaceAll("\\(", " \\( ").replaceAll("\\)"," \\) ");
        ...
        for (String st: stringTokens) {
            ...
            if (matcher.group(TokenType.AND.name()) != null) {
                ...
                tokens.add(new Token(TokenType.LEFTPAREN, matcher.group(TokenType.LEFTPAREN.name())));
                continue;
            } else if (matcher.group(TokenType.RIGHTPAREN.name()) != null) {
                tokens.add(new Token(TokenType.RIGHTPAREN, matcher.group(TokenType.RIGHTPAREN.name())));
                continue;
            	...
            	tokens.add(new Token(TokenType.IP, matcher.group(TokenType.IP.name())));
            	continue;
            } else if (matcher.group(TokenType.ANY.name()) != null) {
            	tokens.add(new Token(TokenType.ANY, matcher.group(TokenType.ANY.name())));
            	continue;
            } else if (matcher.group(TokenType.ALL.name()) != null) {
            	tokens.add(new Token(TokenType.ALL, matcher.group(TokenType.ALL.name())));
            	continue;
