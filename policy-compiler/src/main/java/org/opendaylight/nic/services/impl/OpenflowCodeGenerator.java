 ...
 */
public class OpenflowCodeGenerator implements
        CodeGenerator<Flow, Instruction, MatchBuilder> {

    private static final Logger log = LoggerFactory
            .getLogger(OpenflowCodeGenerator.class);

    /**
     * TODO: 1. Cache per-device capabilities such as matching, masking, tables
     * supported. 2. Refer to cached capabilities to optimize generated flows.
     * 3. Create an internal class which would be created as a sub-generator on
     * a per-device basis. That generator could then have a reference to the
     * device for which it is generating openflow, without the need to pass
     * device ID in each method call. Don't make the public class per-device,
     * because that would require per-device translator registration.
     */
    ...
    public void register(ActionTranslator<Instruction> trans) {
        if (trans.actionLabel() == null)
            throw new IllegalArgumentException(
                    "Translator must return valid label");
        if (ofActionTranslators.containsKey(trans.actionLabel()))
            throw new IllegalArgumentException(
                    "Translator already registered for action: "
                            + trans.actionLabel());
    ...
    public void register(TermTranslator<MatchBuilder> trans) {
        if (ofTermTranslators.containsKey(trans.termLabel()))
            throw new IllegalArgumentException(
                    "Translator already registered for term: "
                            + trans.termLabel());
    ...
    private Set<Instruction> translateAction(Action a) {
        ...
        if (at == null)
            throw new IllegalArgumentException(
                    "No translator registered for action: " + a.label());
    ...
    private void translateTerm(MatchBuilder mb, Term t) {
        ...
        if (tt == null)
            throw new IllegalArgumentException(
                    "No translator registered for term: " + t.typeLabel());
    ...
    /**
     * Generates a set of expressions which include every possible combination
     * of the given terms, according to the restriction that each expression can
     * have only one term of any given type. Starts from the list of existing
     * expressions, and modifies each to include all possible combinations of
     * the given terms.
     *
     * @param termMap
     *            map of term types to terms
     * @return list of modified expres
     */
    private Set<ExpressionImpl> generateExpressions(
            Map<TermType, Set<TermImpl>> termMap) {
        ...
        for (Map.Entry<TermType, Set<TermImpl>> entry : termMap.entrySet()) {
            ...
            for (ExpressionImpl ex : expressions) {
                ...
                for (TermImpl term : terms) {
                    ExpressionImpl modExp = ExpressionImpl.getInstance(ex
                            .getTerms());
                    modExp.addTerm(term);
    ...
    /**
     * Splits a single expression into multiple expressions which perform the
     * same matching, but where each individual expression can be translated
     * into a SINGLE flow.
     *
     * For example, if an expression matched L4_SRC=1,3 and L4_DST=2,4 then we'd
     * split that single expression into 4 different expressions: L4_SRC=1
     * L4_DST=2 L4_SRC=1 L4_DST=4 L4_SRC=3 L4_DST=2 L4_SRC=3 L4_DST=4
     *
     * @param ex
     *            expression to split
    ...
    /**
     * Splits a set of classifier expressions into a possibly larger set which
     * performs the same matching, but does so where each individual expression
     * can be translated into a SINGLE flow.
     *
     * @param expressions
     *            the set of expressions to split
     * @return a split set with the same matching
     */
    private Set<Expression> splitExpressions(
        ...
        for (Expression e : expressions) {
            ...
            if (exp.size() > 100)
                log.debug("Split expression into " + exp.size()
                        + " expressions: " + e);
    ...
    /**
     * Determines whether we'll translate this term during the first pass or
     * second pass.
     *
     * @param term
     *            term to translate
     ...
     *
     * @param c
     *            the classifier to translate
     * @param mb
     *            match builder to use for translation
     */
    ...
    /**
     * Translates the given endpoint to an OpenFlow match, using the given field
     * type as the field against which the group will be matched.
     *
     * @param ep
     *            the endpoint to translate
     * @param type
     *            the type of match
     * @param mb
     *            match builder to use for translation
    ...
    private void matchEndpoint(Endpoint ep, MatchType type, MatchBuilder mb) {
        // Get an L3 match builder, either from scratch or based on the existing
        // one
        ...
        if (mb.getLayer3Match() != null) {
            ...
            if (!(l3m instanceof Ipv4Match))
                throw new IllegalArgumentException(
                        "Cannot match IP endpoint with existing match: " + l3m);
        ...
        if ((ep instanceof IpEndpoint) && ((IpEndpoint) ep).ip() != null) {
            ...
            if (type == MatchType.DST)
                ipv4.setIpv4Destination(new Ipv4Prefix(ipep.ip()
                        .getHostAddress() + "/32"));
            if (type == MatchType.SRC)
                ipv4.setIpv4Source(new Ipv4Prefix(ipep.ip().getHostAddress()
                        + "/32"));
            mb.setLayer3Match(ipv4.build());
        } else {
            log.error("Cannot match IP for endpoint '" + ep.id() + "' of type "
                    + ep.getClass().getName());
        }
    }

    /**
     * Translates a set of source and destination endpoints into a set of
     * matches, based upon the fields already set in the given match builder.
     *
     * @param srcEp
     ...
     */
    private Set<Match> translateEndpoints(Set<Endpoint> srcEp,
            Set<Endpoint> dstEp, MatchBuilder mb) {
    ...
    private Set<Flow> translatePolicyToFlows(CompiledPolicy p) {
        ...
        if (p.actions() != null) { // FIXME: Remove this clause, once compiler
                                   // sends good data.
            for (Set<Action> as : p.actions()) {
                // Translate each action
                for (Action a : as) {
                    log.debug("Translated action " + a + " into "
                            + translateAction(a).size() + " instructions");
                    for (Instruction i : translateAction(a)) {
                        // FIXME: There should be a better way to do things than
                        // the cast on the following line ...
                        ib.getInstruction()
                                .add((org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction) i);
                    }
                }
            }
        }
        ...
        // Some terms cannot be fit into a single flow, even with masking (such
        // as L4 ports=1,3)
        // so we'll do a first pass and split the expressions so that each
        // expression can be
        // represented in a single flow.
        Set<Expression> expressions = splitExpressions(p.classifier()
                .getExpressions());
        log.debug("Split " + p.classifier().getExpressions().size()
                + " expressions into " + expressions.size() + " expressions");

        // If there are no expressions in the list, insert a dummy expression so
        // that we'll
        ...
        // Translate the expressions. We will generate a separate flow for each
        // expression
        // in the classifier, because expressions are logically OR'ed together.
        for (Expression ex : expressions) {
            // Create a match builder which will allow matching both classifier
            // and endpoints.
            ...
            // FIXME: For a classifier with multiple expressions, we're redoing
            // the
            // same computation for EACH expression. Is there a way to only do
            // it once?
            ...
            log.debug("Translated a set of " + p.src().size()
                    + " source endpoints and " + p.dst().size()
                    + " destination endpoints into a set of " + matches.size()
                    + " matches");
            for (Match m : matches) {
