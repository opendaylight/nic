...
public class RedirectTranslator extends BaseActionTranslator {
    ...
    public Set<Instruction> translate(AuxiliaryData aux) {
        ...
        if (!aux.getData().containsKey(REDIRECT_PORT_KEY))
            throw new IllegalArgumentException("Translator requires '"+REDIRECT_PORT_KEY+"' parameter");

        // FIXME: Support multiple inspection points?
        Action allow = new ActionBuilder().setAction(new OutputActionCaseBuilder()
            .setOutputAction(new OutputActionBuilder()
                .setOutputNodeConnector(new Uri(aux.getData().get(REDIRECT_PORT_KEY))).build()).build())
            .build();
