...
public class AuditTranslator extends BaseActionTranslator {
    ...
    public Set<Instruction> translate(AuxiliaryData aux) {
        ...
        Action normal = new ActionBuilder().setAction(new OutputActionCaseBuilder()
            .setOutputAction(new OutputActionBuilder()
                .setOutputNodeConnector(new Uri(OutputPortValues.NORMAL.toString())).build()).build())
            .build();
        Action controller = new ActionBuilder().setAction(new OutputActionCaseBuilder()
            .setOutputAction(new OutputActionBuilder()
                .setOutputNodeConnector(new Uri(OutputPortValues.CONTROLLER.toString())).build()).build())
            .build();
        ApplyActions apply = new ApplyActionsBuilder().setAction(ImmutableList.of(normal,controller))
                .build();
