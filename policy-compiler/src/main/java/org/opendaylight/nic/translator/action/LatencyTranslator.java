...
public class LatencyTranslator extends BaseActionTranslator {
    ...
    public Set<Instruction> translate(AuxiliaryData aux) {
        ...
        if (!aux.getData().containsKey(LATENCY_DSCP_KEY))
            throw new IllegalArgumentException("Translator requires '"+LATENCY_DSCP_KEY+"' parameter");

        short dscpVal = Short.valueOf(aux.getData().get(LATENCY_DSCP_KEY));
        IpMatch dscpIpMatch = new IpMatchBuilder().setIpDscp(new Dscp(dscpVal)).build();
        Match dscpMatch = new MatchBuilder().setIpMatch(dscpIpMatch).build();
        Action latency = new ActionBuilder().setAction(new SetFieldCaseBuilder()
            .setSetField(new SetFieldBuilder(dscpMatch).build())
            .build()).build();
        ApplyActions applyLatency = new ApplyActionsBuilder().setAction(ImmutableList.of(latency))
                .build();
