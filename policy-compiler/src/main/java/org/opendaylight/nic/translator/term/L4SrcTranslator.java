    ...
    public void translate(MatchBuilder mb, List<? extends Interval> intervals) {
        for (Interval i : intervals) {
            ...
            if (l4 instanceof UdpMatch) {
                ...
                l4 = new UdpMatchBuilder(udpMatch).setUdpSourcePort(new PortNumber(i.start())).build();
            }
            else if (l4 instanceof TcpMatch) {
                TcpMatch tcpMatch = (TcpMatch)l4;
                l4 = new TcpMatchBuilder(tcpMatch).setTcpSourcePort(new PortNumber(i.start())).build();
            }
            else
