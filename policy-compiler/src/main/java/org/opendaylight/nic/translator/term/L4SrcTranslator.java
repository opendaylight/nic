...
public class L4SrcTranslator implements TermTranslator<MatchBuilder> {

    private static final TermTypeImpl l4SrcTermType = TermTypeImpl.L4_SRC;
        ...
        for (Interval i : intervals) {
            ...
            if (l4 == null)
                throw new IllegalArgumentException(
                        "Must translate IP protocol before L4 port");

            if (l4 instanceof UdpMatch) {
                UdpMatch udpMatch = (UdpMatch) l4;
                l4 = new UdpMatchBuilder(udpMatch).setUdpSourcePort(
                        new PortNumber(i.start())).build();
            } else if (l4 instanceof TcpMatch) {
                TcpMatch tcpMatch = (TcpMatch) l4;
                l4 = new TcpMatchBuilder(tcpMatch).setTcpSourcePort(
                        new PortNumber(i.start())).build();
            } else
