...
public class StatTranslator extends BaseActionTranslator {
    ...
    public Set<Instruction> translate(AuxiliaryData aux) {
        ...
        Action log = new ActionBuilder().setAction(new OutputActionCaseBuilder()
            .setOutputAction(new OutputActionBuilder()
                .setOutputNodeConnector(new Uri(OutputPortValues.CONTROLLER.toString())).build()).build())
            .build();
        ApplyActions applyLog = new ApplyActionsBuilder().setAction(ImmutableList.of(log))
                .build();

        // Wrap our Apply Action in an Instruction
        org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.Instruction logInstruction = new InstructionBuilder() //
                .setOrder(0)
                .setInstruction(new ApplyActionsCaseBuilder()
                    .setApplyActions(applyLog)
                    .build())
                .build();
    ...
    public ActionLabel actionLabel() {
        return StatActionType.getInstance().label();
    }
