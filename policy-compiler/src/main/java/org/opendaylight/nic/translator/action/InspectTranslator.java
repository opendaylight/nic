...
import org.opendaylight.nic.extensibility.actions.InspectActionType;
...
public class InspectTranslator extends BaseActionTranslator {

    public static final String INSPECT_PORT_KEY = "inspect_port";
    ...
    public Set<Instruction> translate(AuxiliaryData aux) {
        ...
        if (!aux.getData().containsKey(INSPECT_PORT_KEY))
            throw new IllegalArgumentException("Translator requires '"+INSPECT_PORT_KEY+"' parameter");

        // FIXME: Support multiple inspection points?
        Action allow = new ActionBuilder().setAction(new OutputActionCaseBuilder()
            .setOutputAction(new OutputActionBuilder()
                .setOutputNodeConnector(new Uri(aux.getData().get(INSPECT_PORT_KEY))).build()).build())
            .build();
        ApplyActions applyAllow = new ApplyActionsBuilder().setAction(ImmutableList.of(allow))
                .build();
    ...
    public ActionLabel actionLabel() {
        return InspectActionType.getInstance().label();
    }
