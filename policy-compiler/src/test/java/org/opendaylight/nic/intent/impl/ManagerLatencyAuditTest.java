 ...
 * @author Duane Mentze
 */
public class ManagerLatencyAuditTest {
    ...
    public void latencyAuditTest() {
        ...
        network.registerAttribute("authorziedDnsServers");
        ...
        network.addEndpoint("access_sw1", "10.0.0.4");

        // apply attributes to endpoints
        network.addAttribute("10.0.0.1", "employee");
        network.addAttribute("10.0.0.2", "employee");
        network.addAttribute("10.0.0.3", "authorizedServers");
        network.addAttribute("10.0.0.4", "PacketCaptureService");
        ...
        // create policy A
        pr = new PolicyImpl(new PolicyId("A"), "A",
                network.getApplication("app1"), new EndpointGroupImpl(
                        "10.0.0.1"), new EndpointGroupImpl("10.0.0.2"),
                ClassifierHelper.ethType(ETHERTYPE_IPV4),
                ActionHelper.latency(EXPEDITED_FORWARDING), false);
        // add policy A
        ...
        pr = new PolicyImpl(new PolicyId("B"), "B policy",
                network.getApplication("app2"), new EndpointGroupImpl(
                        "10.0.0.1"), new EndpointGroupImpl("employee"),
                ClassifierHelper.classifierEmpty(),
                ActionHelper.audit("PacketCaptureService"), false);
        ...
        List<CompiledPolicy> list = c.compile(
                network.framework().getPolicies(), network.endpointService()
                        .getEndpoints(), network.framework().getActions(),
                network.framework().getTermTypes(), network.deviceService()
                        .getDevices());
