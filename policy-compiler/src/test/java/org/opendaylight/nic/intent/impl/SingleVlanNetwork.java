...
public class SingleVlanNetwork {
	...
	public void addAttribute(String endpointIp, String attribute) {
		EndpointId id;
		try {
			id = new IpEndpointId(InetAddress.getByName(endpointIp), new SegmentId(vlan()) );
	...
	public void addEndpoint(String device, String endpointIp) {
		...
		IpEndpoint ep = makeEndpoint(endpointIp, Integer.valueOf(1), vlan(), deviceService.getDevices().get(id));
	...
	private static IpEndpoint makeEndpoint(String ip, Integer port, Integer vlan, Device device) {
		IpEndpoint ep1 = null;
		try {
